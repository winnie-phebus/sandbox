<script>
  import RoundHeart from "./assets/base/svg/round_heart.svelte";
  import CollapsibleControl from "./lib/CollapsibleControl.svelte";
  import Reset from "./assets/icon/reset.svelte";
...
<main>
  {#snippet heartDataValue(value, label = "")}
    <div class="hData">
  ...
  {#snippet resetButton(adjustible, key, defaultVal)}
    <button class="reset" onclick={() => (adjustible[key] = defaultVal)}
      ><Reset color={"red"} width={20} height={20} />
    </button>
  {/snippet}

  {#snippet numberAdjuster(adjustible, key, { defaultVal, min, max, step })}
  ...
  {#snippet colorAdjuster(adjustible, key, { defaultVal })}
    {@render resetButton(adjustible, key, defaultVal)}
